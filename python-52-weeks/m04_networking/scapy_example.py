...
print("\n----- Packet summaries --------------------")
capture = scapy.sniff(iface='wlp0s20f3', count=10)
print(capture.nsummary())

# CAPTURE DNS AND PRINT PACKETS
print("\n----- DNS packet summaries (collect 10 DNS packets) --------------------")
capture = scapy.sniff(iface='wlp0s20f3', filter="udp port 53", count=10)
print(capture.nsummary())
#
...
#
#
scapy.sniff(iface='wlp0s20f3', prn=print_packet, filter="tcp port https", count=10)
#
#
# CAPTURE AND HANDLE PACKETS AS THEY ARRIVE USing LAMBDA
print("\n\n----- Capture and print packets as sniffed (using lambda) ---------------------")
scapy.sniff(iface='wlp0s20f3', prn=lambda pkt: print(f"lambda    {pkt.summary()}"), filter="tcp port https", count=10)
